...
ClassCEndDeviceLorawanMac::TxFinished (Ptr<const Packet> packet)
{
  ...
  OpenSecondReceiveWindow(true); //After the uplink is send a short RX2 opens followed by a short RX1 and then a continuous RX2. 
  ...
  Simulator::Schedule (m_receiveDelay2, &ClassCEndDeviceLorawanMac::OpenSecondReceiveWindow, this,false); 
}
...
{
  NS_LOG_FUNCTION_NOARGS ();
  NS_LOG_FUNCTION(m_closeFirstWindow.IsRunning() << m_closeSecondWindow.IsRunning() <<m_secondReceiveWindow.IsRunning()<<waitingTime );
